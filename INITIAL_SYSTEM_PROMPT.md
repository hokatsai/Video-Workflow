DEPRECATED - do not run. Use AGENTS.md for runtime rules.

## My request for Codex:
# INITIAL_SYSTEM_PROMPT v6 — One-Shot Installer (Use & Discard)
**Goal:** run this prompt exactly once to scaffold the local-first agent workspace and migrate the canonical runtime spec into `AGENTS.md`. All future sessions must read only `AGENTS.md`.

**Baseline assumptions:** interact via natural language, prefer existing assets, keep changes minimal, and surface results quickly. User-facing docs remain in **Traditional Chinese（繁體中文）**.

---

## Installer Tasks (run once, then stop)
1. **Scaffold** the repository structure if anything is missing (see tree below).
2. **Write `AGENTS.md`** by copying the canonical spec verbatim from `## Agent Runtime Rules` onward. Exclude this installer section.
3. **Seed required stubs** so the workspace is ready for tooling, prompts, and documentation:
   - `01-system/configs/env.example`
   - `01-system/configs/apis/{README.md, API-Keys.md}` (human-filled key store)
   - `01-system/configs/tools/registry.yaml`
   - `01-system/docs/agents/{PLAYBOOKS.md, TOOLS.md, TROUBLESHOOTING.md, SYSTEM_MEMORY.md, STATE.md, BOOTSTRAP.md, memory/YYYY-MM.md}`
   - `01-system/docs/prompts/{README.md, INDEX.md, examples/prompt-template.md}`
   - `01-system/docs/user/{README.md, INDEX.md}` and `01-system/docs/user/tools/`
   - `01-system/tools/{ops/, llms/, stt/, _categories-README.md}`
   - `02-inputs/` (+ `downloads/` if needed for ingestion flows)
   - `03-outputs/README.md` and representative tool folders under `03-outputs/<tool>/`
4. **Load secrets** from `01-system/configs/apis/API-Keys.md` into the process environment only when required. Never log secret values.
5. **Record the bootstrap** in `SYSTEM_MEMORY.md` (Lean Logflow standard run), update `STATE.md` with phase, next steps, and artifact pointers, and append a timestamped note to `01-system/docs/agents/BOOTSTRAP.md`. Call out any `03-outputs/<tool>/...` paths created.
6. **Deprecate this file** (e.g., prepend `DEPRECATED — do not run` or move it under `01-system/docs/agents/BOOTSTRAP.md#installer`).
7. **Stop**. Report what was created or verified; all future sessions start from `AGENTS.md`.
...
```
/
├─ AGENTS.md                     # canonical runtime spec copied from this installer
├─ INITIAL_SYSTEM_PROMPT.md      # one-shot installer (deprecate after bootstrap)
├─ Makefile
├─ 01-system/
│  ├─ configs/
│  │  ├─ env.example
│  │  ├─ apis/{README.md, API-Keys.md}
│  │  └─ tools/registry.yaml
│  ├─ docs/
...
```
Artifacts live under `03-outputs/<tool>/...`.

### `API-Keys.md` minimal format (human-editable)
The agent parses `KEY=VALUE` lines and ignores comments or blanks. Store only redacted placeholders in version control.
```md
...
### `registry.yaml` seed
Keep the registry authoritative; every runnable tool must appear here.
```yaml
...
## Canonical Spec for `AGENTS.md`
Copy everything from the next heading through "End of canonical spec." into `AGENTS.md` without alteration.## Agent Runtime Rules
### Purpose
...
2. **Assets moved or added?** → When you add/modify a tool wrapper or prompt file, update in the same work block: `registry.yaml` → `docs/agents/TOOLS.md` → `docs/prompts/INDEX.md` (for prompts) → related playbook entries → user docs in 繁體中文 (as applicable).
